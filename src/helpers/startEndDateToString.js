...
function dateCompare(inicio, fim){
	...
	result += `${inicio.getDate()}`;

	if((inicio.getMonth() === fim.getMonth()) && (inicio.getDate() === fim.getDate())){
		result += ` de `;
	}else if((inicio.getMonth() === fim.getMonth() && inicio.getFullYear() === fim.getFullYear()) && (inicio.getDate() !== fim.getDate())){
		result += ` a `;
		result += `${fim.getDate()} de `;
	}
...
function data_to_inline_number(data){
	return data.getFullYear()*10000 + (data.getMonth()*1000) + data.getDate();
}
function data_time_to_inline_number(data){
	let time = data.getHours()*100 + (data.getMinutes());
	return data.getFullYear()*100000000 + (data.getMonth()*1000000) + data.getDate()*10000+time;
}